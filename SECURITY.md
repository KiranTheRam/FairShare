...
- Independent login throttling by normalized email and trusted client IP, with constant-work password verification for unknown accounts.
...
- Push subscriptions scoped to their authenticated owner and restricted to configured browser push providers with bounded outbound requests.
- Private, non-cacheable API responses and no authenticated HTML in the service-worker cache.
- Per-request nonce CSP, clickjacking, MIME-sniffing, referrer, permissions, COOP/CORP, and HSTS headers.
- Recipient-confirmed, idempotent payments with transaction and row locking around balance changes.
...
The nonce-based Content Security Policy dynamically renders application pages so every framework script receives a per-request nonce. If an edge proxy injects scripts into HTML after it leaves the application, configure that feature to preserve or honor the application CSP rather than weakening the policy globally.
