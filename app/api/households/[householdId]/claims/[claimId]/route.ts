...
export async function PATCH(request: NextRequest, context: { params: Promise<{ householdId: string; claimId: string }> }) {
  return apiRoute(async () => {
    ...
    if (input.amountCents !== undefined) {
      const outstanding = await outstandingForPair(householdId, claim.debtorUserId, claim.creditorUserId);
      if (outstanding <= 0) throw new ApiError(400, "You have no outstanding balance with this member", "no_outstanding_balance");
      if (input.amountCents > outstanding) throw new ApiError(400, "The claim is larger than what you owe", "claim_exceeds_balance");
    }
