...
import { ArrowDownLeft, ArrowRight, Bell, BellRing, CalendarClock, Check, Copy, CreditCard, Download, FileText, LayoutDashboard, Menu, MessageSquare, Moon, Paperclip, Plus, ReceiptText, Settings, Sun, Trash2, Trees, UserPlus, WalletCards, X } from "lucide-react";
...
type ModalName = "bill" | "edit" | "payment" | "recurring-edit" | "detail" | "settings" | "invite" | null;
...
function Overview({ data, user, net, canConfirmPayment, onBill, onPayment, onInvite, onBillDetail }: { data: HouseholdData; user: User; net: number; canConfirmPayment: boolean; onBill: () => void; onPayment: () => void; onInvite: () => void; onBillDetail: (id: string) => void }) {
  return <div className="dashboard-grid"><section className="summary-card"><div className="summary-top"><span className="summary-icon"><WalletCards size={21} /></span><span>Your overall position</span></div><p>{net < 0 ? "You owe" : "You are owed"}</p><h2>{money(Math.abs(net), data.household.currency)}</h2><span className="delta">Across {data.balances.length} open balance{data.balances.length === 1 ? "" : "s"}</span><div className="summary-divider" />{data.balances.filter((item) => item.payerUserId === user.id || item.recipientUserId === user.id).slice(0, 3).map((item) => <div className="mini-balance" key={`${item.payerUserId}:${item.recipientUserId}`}><div className="avatar-pair"><Avatar name={item.payerName ?? ""} /><Avatar name={item.recipientName ?? ""} /></div><span><small>{item.payerUserId === user.id ? `You owe ${item.recipientName}` : `${item.payerName} owes you`}</small><strong>{money(item.amountCents, data.household.currency)}</strong></span></div>)}</section>
    ...
    <section className="quick-card"><div><span className="quick-icon"><CreditCard size={22} /></span><span><strong>Keep the ledger current</strong><small>Add an expense or confirm money you received.</small></span></div><div><button className="quick-action" onClick={onBill}><Plus size={18} /><span><strong>Add a bill</strong><small>Split an expense</small></span></button><button className="quick-action" onClick={onPayment} disabled={!canConfirmPayment}><CreditCard size={18} /><span><strong>Confirm payment</strong><small>Money you received</small></span></button><button className="quick-action" onClick={onInvite}><UserPlus size={18} /><span><strong>Invite a roommate</strong><small>Share a join link</small></span></button></div></section></div>;
}

function BillRow({ bill, currency, onClick }: { bill: Bill; currency: string; onClick?: () => void }) { const settled = bill.status === "settled"; return <button className="bill-row" onClick={onClick}><span className={`activity-icon ${settled ? "mint" : "coral"}`}>{settled ? <Check size={18} /> : <ReceiptText size={18} />}</span><span className="bill-main"><strong>{bill.name}</strong><small>{bill.periodLabel} · {CATEGORY_LABELS[bill.category] ?? CATEGORY_LABELS.other} · {bill.amountState}</small></span><span className="bill-amount"><strong>{money(bill.amountCents, currency)}</strong><span className={`status-pill ${settled ? "done" : "open"}`}>{settled ? "Settled" : "Outstanding"}</span></span></button>; }
function Bills({ data, onBillDetail }: { data: HouseholdData; onBillDetail: (id: string) => void }) {
  ...
  const bills = data.bills.filter((bill) => bill.status === filter);
  const openCount = data.bills.filter((bill) => bill.status === "open").length;
  const settledCount = data.bills.filter((bill) => bill.status === "settled").length;
  ...
  return <section className="page-content bills-view"><div className="page-toolbar"><p>{filter === "open" ? "Select an expense to settle each person’s share." : "Completed expenses remain available for review."}</p><div className="filter-pills bill-status-filter"><button className={filter === "open" ? "selected" : ""} onClick={() => setFilter("open")}>Outstanding · {openCount}</button><button className={filter === "settled" ? "selected" : ""} onClick={() => setFilter("settled")}>Settled · {settledCount}</button></div></div><div className="month-total"><span><small>OUTSTANDING</small><strong>{openCount}</strong></span><span><small>SETTLED</small><strong>{settledCount}</strong></span><span><small>RECORDED SPEND</small><strong>{money(data.bills.reduce((sum, bill) => sum + bill.amountCents, 0), data.household.currency)}</strong></span><span><small>OPEN BALANCES</small><strong>{money(data.balances.reduce((sum, item) => sum + item.amountCents, 0), data.household.currency)}</strong></span></div>
    {categoryTotals.length > 0 && <div className="category-breakdown"><p className="eyebrow">SPENDING BY CATEGORY</p>{categoryTotals.map(([category, amountCents]) => <div className="category-line" key={category}><span>{CATEGORY_LABELS[category] ?? CATEGORY_LABELS.other}</span><span className="category-bar"><span style={{ width: `${spendTotal ? Math.max(3, Math.round(amountCents / spendTotal * 100)) : 0}%` }} /></span><strong>{money(amountCents, data.household.currency)}</strong></div>)}</div>}
    <div className="bill-list flat-list">{bills.map((bill) => <BillRow key={bill.id} bill={bill} currency={data.household.currency} onClick={() => onBillDetail(bill.id)} />)}{!bills.length && <p className="empty-copy">{filter === "open" ? "No outstanding expenses. Everyone is settled up." : "No settled expenses yet."}</p>}</div></section>;
}
