...
// A read-only suggestion that minimizes the number of repayments needed to clear
// every pairwise balance. It never replaces the underlying pairwise ledger: each
// member's net position is preserved exactly, but money may flow along different
// pairs than the original obligations.
export function simplifyBalances(balances: Array<{ payerUserId: string; recipientUserId: string; amountCents: number }>): Transfer[] {
  const net = new Map<string, number>();
  ...
  const creditors = [...net].filter(([, amount]) => amount > 0).map(([userId, amount]) => ({ userId, amount })).sort((a, b) => b.amount - a.amount || a.userId.localeCompare(b.userId));
  const debtors = [...net].filter(([, amount]) => amount < 0).map(([userId, amount]) => ({ userId, amount: -amount })).sort((a, b) => b.amount - a.amount || a.userId.localeCompare(b.userId));
  const transfers: Transfer[] = [];
  let creditorIndex = 0;
  let debtorIndex = 0;
  while (creditorIndex < creditors.length && debtorIndex < debtors.length) {
